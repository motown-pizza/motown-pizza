...
const trackingHash = async (
  ...
  try {
    ...
    for (let i = 0; i < length; i++) {
      ...
      result += TRACKING_ALPHABET[byte % TRACKING_ALPHABET.length];
    }
...
export const storeTitleToCode = (title: string, length: number = 3): string => {
  ...
  if (words.length === 2) {
    ...
    return (firstChar + secondChar + secondChar).toUpperCase();
  }

  if (words.length === 1) {
    const word = (words[0] ?? '').toUpperCase();
    const fallbackChar = word[0] ?? 'X';
    return word.length >= length ? word.slice(0, length) : word.padEnd(length, fallbackChar);
  }
...
export const generateTrackingCode = async (params: {
  ...
  const delivery = (params.deliveryType[0] ?? 'X').toUpperCase();
