 ...
 */
export const isFirstItem = <T>(array: T[], item: T): boolean => array[0] === item;

/**
 * Checks if the given item is the last element in the array.
 */
export const isLastItem = <T>(array: T[], item: T): boolean => array[array.length - 1] === item;
...
export const sortArray = <T, K>(
  ...
  const [valid, invalid] = array.reduce<[T[], T[]]>(
    ([v, n], item) => (getField(item) == null ? [v, [...n, item]] : [[...v, item], n]),
    [[], []],
  );
  ...
  const sortedValid = [...valid].sort((a, b) => {
    ...
    throw new Error(`Cannot compare values of types ${typeof aValue} and ${typeof bValue}`);
  });
