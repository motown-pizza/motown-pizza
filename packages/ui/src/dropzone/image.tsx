...
import { FormProduct } from '@repo/hooks';
import { generateUUID } from '@repo/utils';
import { imageUpload } from '@repo/uploads';
...
export function DropzoneImage({
  ...
  const [status, setStatus] = useState<'accept' | 'loading' | 'reject' | 'idle'>('idle');
  ...
  const previews = files.map((file, i) => {
    ...
    return <ImageComponent key={i} props={{ src: imageUrl, alt: file.name }} {...imageProps} />;
  });
...
const reject = (
  <div>
    <Group justify="center">
      <IconX size={ICON_SIZE * 2.5} color="var(--mantine-color-red-6)" stroke={ICON_STROKE_WIDTH} />
    </Group>
...
const ImageComponent = ({ props, ...restProps }: { props: { src: string; alt: string } }) => {
  return (
    <ImageDefault
      key={'default'}
      src={props.src}
