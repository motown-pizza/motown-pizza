...
export function FormStoresOrder({
  ...
  const orderItemsCurrent = orderItems?.filter((oi) => oi.orderId == props?.defaultValues?.id);
                      ...
                      <GridCol span={{ base: 12, xs: 6 }}>
                        <Box mih={60.8} mt={'xs'}>
                          <Checkbox
                            ...
                            checked={withGuests}
                            onChange={(event) => setWithGuests(event.currentTarget.checked)}
                          />
                        </Box>
                      </GridCol>
