...
import { useFormProductVariant } from '@repo/hooks';
import {
  Button,
  Center,
  Divider,
...
import { CardRecipieItem } from '../../card/recipie-item';

export function FormStoresProductVariant({
  props,
}: {
  props?: {
  ...
  const product = products?.find((p) => p.id == props?.defaultValues?.productId);

  const { recipieItems } = useStoreRecipieItem();

  const productVarintRecipieItems = recipieItems?.filter(
    (ri) => ri.productVariantId == props?.defaultValues?.id,
  );
      ...
      <Grid gap={mobile ? 5 : undefined}>
        ...
        <GridCol span={6}>
          <NumberInput
            ...
            placeholder="Price"
            min={0}
            leftSection={<IconPremiumRights size={ICON_SIZE} stroke={ICON_STROKE_WIDTH} />}
                ...
                <Stack gap={0}>
                  ...
                  >
                    ...
                    <ModalCrudRecipieItem
                      props={{
                        defaultValues: {
                          productVariantId: props.defaultValues.id || '',
                        },
                      }}
                    >
                      <Button
                        size="xs"
                        variant="default"
                        leftSection={<IconPlus size={ICON_SIZE - 4} stroke={ICON_STROKE_WIDTH} />}
                      >
                        Add Recipie Item
                      </Button>
                    </ModalCrudRecipieItem>
        ...
        <GridCol span={12}>
          <Group justify="end" mt={mobile ? 'xs' : undefined}>
            <Button fullWidth type="submit" loading={submitted}>
              {!props?.defaultValues?.updatedAt ? 'Add' : 'Save'}
            </Button>
          </Group>
        </GridCol>
      </Grid>
