...
import { useAppshellNavbar } from '@repo/hooks';
...
export function ButtonAppshellNavbar() {
  ...
  if (appshell === undefined) return <Skeleton h={ICON_WRAPPER_SIZE} w={ICON_WRAPPER_SIZE} />;

  return (
    <Group>
      <Tooltip label={appshell?.navbar ? 'Collapse' : 'Expand'} position="right">
        <Group>
          <ActionIcon
            variant="subtle"
            color={'gray'}
