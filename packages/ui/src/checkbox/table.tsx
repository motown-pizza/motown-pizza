      ...
      {props.list === undefined ? (
          ...
          onChange={(event) =>
            props.setSelectedRows(
              event.currentTarget.checked
                ...
                : props.selectedRows.filter((item) => item !== props.options?.itemId),
            )
          }
        />
      ) : (
        <Tooltip label={`Select/Deselect all`}>
          <Checkbox
            ...
            checked={
              (props.rows || []).length > 0 && props.selectedRows.length == props.list?.length
            }
            onChange={(event) =>
              props.setSelectedRows(
                event.currentTarget.checked ? (props.list || []).map((p) => p.id) : [],
              )
            }
          />
        </Tooltip>
