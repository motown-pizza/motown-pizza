          ...
          >
            ...
            <Card padding={0} bg={'transparent'} radius={0}>
              <Group justify="space-between" gap={'xs'}>
                <Avatar
                  key={!orderDetails ? 'Customer Name' : orderDetails.customerName}
                ...
                <Stack gap={0} justify="center" mih={43.3}>
                  <Title order={2} fz={'md'} fw={'bold'} miw={160}>
                    {!orderDetails ? 'Customer Name' : orderDetails.customerName}
                  </Title>
...
function CardOrderDetails() {
  ...
  const handlePlaceOrder = () => {
    ...
    router.push(`/pos/order-confirmed?${PARAM_NAME.ORDER_CONFIRMED}=${orderDetails.id}`);
  };
        ...
        <Group justify="space-between" align="start">
          <Stack gap={0} mih={40.2} justify="space-between">
            <Title order={2} fz={'md'} fw={500}>
              {!orderDetails ? 'Customer Name' : orderDetails.trackingCode}
            </Title>
...
function CardMenuItemGroup({
  ...
  switch (props.type) {
    ...
    case ProductType.SIDE:
      ...
      cardProps.length = products?.filter((pi) => pi.type == ProductType.SIDE).length || 0;
      break;
    case ProductType.SOUP:
      ...
      cardProps.length = products?.filter((pi) => pi.type == ProductType.SOUP).length || 0;
      break;
    case ProductType.DESSERT:
      ...
      cardProps.length = products?.filter((pi) => pi.type == ProductType.DESSERT).length || 0;
      break;
    case ProductType.SALAD:
      ...
      cardProps.length = products?.filter((pi) => pi.type == ProductType.SALAD).length || 0;
      break;
