...
import { CardOverviewMain } from '@repo/ui';
import { IconCoin, IconMoodPuzzled, IconToolsKitchen } from '@tabler/icons-react';
...
import { OrderStatus, ProductType } from '@repo/types';
...
import { useTime } from '@repo/hooks';
...
function CardRecentOrders() {
  ...
  const filteredOrders = orders?.filter((oi) => oi.orderStatus != OrderStatus.DRAFT);
          ...
          >
            <Group justify="space-between" align="end">
              ...
              <Group justify="end" gap={'xs'}>
                ...
                <AnchorNextLink href="/pos/orders">
    ...
    <Card bg={'transparent'} padding={0} px={0} py={5}>
      <Group justify="space-between">
        <Group gap={'xs'}>
          ...
          <div>
            <Text inherit>{capitalizeWords(props.customerName)}</Text>
...
function CardPopularItems() {
  ...
  const pizzaVariants = productVariants?.filter((pv) => {
    const pizzaIds = products?.filter((p) => p.type == ProductType.PIZZA).map((p) => p.id);

    return pizzaIds?.includes(pv.productId);
  });

  const dedupedPizzaVariants = Array.from(
    new Map(pizzaVariants?.map((i) => [i.productId, i])).values(),
  );
...
function CardPopularItem({ props }: { props: ProductVariantGet }) {
  ...
  const productRecipieItems = recipieItems?.filter((ri) => ri.productVariantId == props.id);

  const productIngredients = ingredients?.filter((i) =>
    productRecipieItems?.map((ri) => ri.ingredientId).includes(i.id),
  );
  ...
  const timesOrdered = orderItems?.filter((oi) => {
    const productVariantsCurrent = productVariants
      ?.filter((pvi) => pvi.productId == product?.id)
      .map((pvi2) => pvi2.id);

    return productVariantsCurrent?.includes(oi.productVariantId);
  }).length;
