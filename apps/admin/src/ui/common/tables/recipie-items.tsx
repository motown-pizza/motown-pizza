...
export default function RecipieItems({ props }: { props?: { recipieItems?: RecipieItemGet[] } }) {
  const { ingredients } = useStoreIngredient();
  const { productVariants } = useStoreProductVariant();
  const { recipieItems, setRecipieItems, deleteRecipieItems } = useStoreRecipieItem();
  ...
  const rows = items.map((p) => {
    ...
    const productVariant = productVariants?.find((i) => i.id == p.productVariantId);
          ...
          <Stack gap={0}>
            <Text>
              <Text component="span" inherit>
                <NumberFormatter value={p.quantityNeeded} />
              </Text>{' '}
          ...
          <>
            ...
            <ButtonDelete
              props={{
                onConfirm: () => {
                  deleteRecipieItems(
                    (recipieItems || []).filter((i) => selectedRows.includes(i.id)),
                  );
                },
              }}
            />
