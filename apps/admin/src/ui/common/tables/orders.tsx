...
import { OrderFulfilmentType, OrderSource, OrderStatus } from '@repo/types';
...
export default function Orders() {
  ...
  const filteredItems = orders?.filter((oi) => oi.orderStatus != OrderStatus.DRAFT);
  ...
  const rows = items.map((p) => {
    const createdAt = getRegionalDate(p.createdAt);
    const orderItemsCurrent = orderItems?.filter((oi) => oi.orderId == p.id);
      ...
      >
        ...
        <TableTd w={widths.title}>
          <Group gap={'xs'} wrap="nowrap">
            <Avatar key={p.customerName} name={p.customerName} size={36} color="initials" />

            <Stack gap={0} align="start">
              <Tooltip label={p.customerName} multiline maw={240}>
                <Title order={3} fz={'md'} fw={500} lineClamp={1}>
                  {p.customerName}
                </Title>
              </Tooltip>

              <Tooltip label={p.customerPhone} multiline maw={240}>
                <Text c={'dimmed'} fz={'sm'} lineClamp={1}>
        ...
        <TableTd w={widths.actions}>
          <Group gap={'xs'} justify="end" wrap="nowrap">
            ...
            <ModalConfirm
              props={{
                title: `Delete Order`,
                desc: `This will remove all data associated with the order (${p.trackingCode}). This action is irreversible.`,
                onConfirm: () => orderDelete(p),
                confirmMessage: `(${p.trackingCode}) and all data associated with it has been removed.`,
              }}
            >
              <Group>
          ...
          <>
            ...
            {selectedRows.length > 0 && (
              <ButtonDelete
                props={{
                  onConfirm: () => {
                    deleteOrders((orders || []).filter((i) => selectedRows.includes(i.id)));
                  },
                }}
              />
            )}
